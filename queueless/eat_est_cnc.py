...
while minute <= total_minutes or queue_length > 0:
    if minute <= total_minutes and next_arrive_flag:
        times_counter += 1
        time_between_arrivals = random.expovariate(TIME_BETWEEN_ARRIVALS_RATE)
    ...
    if math.fabs(minute - next_arrival_time) < 1.0:
        arrival_counter += 1
